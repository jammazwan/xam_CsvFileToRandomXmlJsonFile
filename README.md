...
### CsvFileToRandomXmlJsonFile 
##### _a Camel example project_

Functionality: **_Show off project only. Generates random XML and JSON files from CSV input_**
...
git clone https://github.com/jammazwan/xam_CsvFileToRandomXmlJsonFile.git
cd jammazwan.shared
mvn install 
```
...
_Too complex for useful x_project example. Created to generate files for other x_projects_
...
so you can focus only on what is required to demo the **CsvFileToRandomXmlJsonFile** feature **_in isolation_**.
...
The prefix **xam** is used for maintaining sort order in a directory, providing packaging, and is otherwise meaningless. The x stands for _example_.
